...
module "lab_k3s_0" {
  ...
  connection_info = {
    user = data.terraform_remote_state.infra.outputs.instances[0]["instance_user"]
...
module "lab_k3s_1" {
  ...
  api_host = module.tailscale_lab_k3s_0.ipv4_address
  connection_info = {
    user = data.terraform_remote_state.infra.outputs.instances[1]["instance_user"]
