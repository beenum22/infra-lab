...
locals {
  install = {
    oracle = join(";", [
      ...
      "if ${var.use_sudo ? "sudo " : ""}test -f '/var/lib/tailscale/tailscaled.state'; then ${var.use_sudo ? "sudo " : ""}tailscale up --auth-key=${var.authkey}; else ${var.use_sudo ? "sudo " : ""}tailscale up --auth-key=${var.authkey} --accept-dns=true --accept-routes=true; fi"
    ])
  }
  upgrade = {
